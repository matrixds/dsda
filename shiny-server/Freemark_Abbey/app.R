...
########################################################################################
# To host a Shiny App, we need to let the Shiny server
# know where the libraries from Rstudio are installed
########################################################################################

.libPaths( c( .libPaths(), "/srv/R/library") )

########################################################################################
# Once Shiny knows where to look, we can specify the needed libraries
########################################################################################
...
ui <- dashboardPage(skin = "green",
                    dashboardHeader(title = "Decision Support Tool ODSC 2019"),
                    dashboardSidebar(
                      collapsed = TRUE,
                      sidebarMenu(
                              ...
                              fluidRow(
                                tabBox(width=4,
                                       ...
                                       tabPanel("Probabilities", 
                                                sliderInput("PRCP", "Precipitation last week ODSC 2019", 0,2,0.5, step = 0.1),
